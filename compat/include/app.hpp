...
static const float SVG_DPI_VCV = 75.0;
static const float MM_PER_IN_VCV = 25.4;
...
inline float in2px(float inches) {
	return inches * SVG_DPI_VCV;
}

inline Vec in2px(Vec inches) {
	return inches.mult(SVG_DPI_VCV);
}

inline float mm2px(float millimeters) {
	return millimeters * (SVG_DPI_VCV / MM_PER_IN_VCV);
}

inline Vec mm2px(Vec millimeters) {
	return millimeters.mult(SVG_DPI_VCV / MM_PER_IN_VCV);
}
